...
import type { ZoraProfileResponse } from '../types/index.js';
...
export class ZoraAPI {
  ...
  async getCreatorInfo(tokenName?: string, tokenSymbol?: string): Promise<{ username: string; followers: number } | null> {
  ...
  private async getCreatorProfile(profileId: string): Promise<{ username: string; followers: number } | null> {
    ...
    try {
      ...
      const response = await this.client.post<ZoraProfileResponse>('', query);
